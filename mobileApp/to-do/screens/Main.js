...
const Main = () => {
    ...
    const fetchActivities = async () => {
        try {
            ...
            const response = await axios.get("http://10.203.228.166:5000/activities", {
                headers: {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (editActivity?.id) {
                await axios.put(`http://10.203.228.166:5000/activities/${editActivity.id}`, editActivity, config);
                Alert.alert("Success", "Activity updated");
            } else {
                const response = await axios.post("http://10.203.228.166:5000/activities", editActivity, config);
    ...
    const handleDelete = async (id) => {
        try {
            const token = await SecureStore.getItemAsync("token");
            await axios.delete(`http://10.203.228.166:5000/activities/${id}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
